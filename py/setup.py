...
"""Build unimcp._core, a Cython extension over the UniMCP C ABI.
...
PKG_DIR = os.path.join(HERE, "unimcp")
VENDOR_DIR = os.path.join(HERE, "_nimsrc")
NIMBLE_FILE = "UniMCP.nimble"
...
if sys.platform == "win32":
    LIB_NAME, BUNDLED = "UniMCP.lib", False
    LINK_ARGS, NIMBLE_TASK = [], "clibMsvc"
elif sys.platform == "darwin":
    LIB_NAME, BUNDLED = "libUniMCP.dylib", True
    LINK_ARGS, NIMBLE_TASK = ["-Wl,-rpath,@loader_path"], "clib"
else:
    LIB_NAME, BUNDLED = "libUniMCP.so", True
...
def nim_project_dir():
    """Where UniMCP.nimble lives: the real repo root in a normal
...
def ensure_lib_built():
    ...
    try:
        ...
        raise SystemExit(
            "setup.py: `nimble` not found on PATH. Building unimcp from "
...
# The sdist ships the pre-transpiled unimcp/_core.c, not the .pyx (Cython
...
# needing no Cython.
pyx = os.path.join("unimcp", "_core.pyx")
ext = Extension(
    "unimcp._core",
    sources=[pyx if os.path.exists(os.path.join(HERE, pyx)) else os.path.join("unimcp", "_core.c")],
...
setup(
    ...
    package_data={"unimcp": [LIB_NAME] if BUNDLED else []},
    exclude_package_data={"unimcp": ["_core.c"]},
    zip_safe=False,
)
